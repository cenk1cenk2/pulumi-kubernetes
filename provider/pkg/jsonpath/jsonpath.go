...
func (i *Parsed) Matches(uns *unstructured.Unstructured) (bool, string, error) {
	results, err := i.parser.FindResults(uns.Object)
	if err != nil {
		return false, "", fmt.Errorf("matching JSONPath: %w", err)
	}
	...
	case []any, map[string]any:
		return false, "", fmt.Errorf("%q has a non-primitive value (%v)", i.path, value.String())
	}
	found := fmt.Sprint(value.Interface())
	return i.value == found, found, nil
}
